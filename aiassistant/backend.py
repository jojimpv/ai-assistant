...
def process_uploads(name: str, file_content) -> int:
    """Write file_content to settings.UPLOADS_DIR directory and then update upload_stats and audit tables.
...
async def process_parse_form(form_id: int) -> None:
    """Trigger form parsing using API call to "/api/parse_form/" and update audit records and `parse_stats` table.
...
async def process_qa_form(form_id: int) -> None:
    """Trigger form QA using API call to "/api/qa_form/" and update audit records and `qa_stats` table.
