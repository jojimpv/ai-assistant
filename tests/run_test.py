...
def get_basic_user_profile() -> str:
    """Get basic user profile which is name, address and job title.

    Returns:
        User profile as new line separated string
...
def get_prompt_for_llm(user_demographics: str, gov_form_content: str) -> str:
    """Create LLM prompt by substituting user demographics and form content in settings.INTERVIEWER_PROMPT.

    Args:
        user_demographics: User demographics
        gov_form_content: Form content
...
def get_evaluation_dataset() -> dict:
    """Get evaluation dataset with help of dummy user profile and test form content.

    Returns:
        Dict with QA generated by LLM
    """
...
def populate_evaluation_dataset(form_id: int) -> None:
    """Populate evaluation dataset to persistent DB tables (upload_stats, parse_stats and qa_stats) and vector DB.
...
def add_reference_form_stats() -> int:
    """Add dummy reference form stats to upload_stats and audit tables.
...
def read_form_from_file(file_path: str) -> tuple[str, bytes]:
    """Read form content from the file_path given.

    Args:
        file_path: File path

    Returns:
        Tuple with file name and file content
...
def setup() -> int:
    """Delete entries from vector DB collection and drop persistent DB tables.
...
def backup_db(form_id: int) -> None:
    """Copy DB file backup to settings.TEST_RESULTS_PATH
